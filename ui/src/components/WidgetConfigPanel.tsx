                ...
                {field.options?.map(o => {
                    ...
                    return (
                        <button
                            ...
                            style={{
                                background: active ? 'color-mix(in srgb, var(--color-cyan) 13%, transparent)' : 'var(--color-surface)',
    ...
    if (field.type === 'toggle') {
        ...
        return (
            <button
                onClick={() => onChange(!on)}
                style={{
                    width: 44, height: 22, background: on ? 'var(--color-cyan)' : 'var(--color-border)',
            ...
            >
                <span
                    style={{
                        width: 16, height: 16, background: 'var(--color-bg)',
            ...
            <div ref={symbolRef} style={{ position: 'relative' }}>
                ...
                {symbolOpen && filtered.length > 0 && (
                    <div style={{
                        ...
                        background: 'var(--color-surface)', border: '1px solid var(--color-border)', maxHeight: 140, overflowY: 'auto',
                    }}>
                        {filtered.map(s => (
                            <div
                                key={s}
    ...
    if (field.type === 'color') {
        return (
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                {/* <input type="color"> only accepts literal hex — no tokens here */}
                <input
                    type="color"
    ...
    if (field.type === 'sliderRange') {
        ...
        return (
            <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                ...
                />
                <span style={{ color: 'var(--color-text-muted)', fontSize: 11, fontFamily: 'monospace', minWidth: 24 }}>{pair[0]}</span>
                <input
                    type="range"
                    min={field.min ?? 0} max={field.max ?? 100} step={field.step ?? 1}
                    value={pair[1]}
                    ...
                    style={{ flex: 1, accentColor: 'var(--color-cyan)' }}
                />
                <span style={{ color: 'var(--color-text-muted)', fontSize: 11, fontFamily: 'monospace', minWidth: 24 }}>{pair[1]}</span>
            </div>
        );
    }
    ...
    return (
        ...
        >
            ...
            <div style={{
                ...
                {IconComp && <IconComp size={16} color="var(--color-cyan)" />}
                <span style={{ color: 'var(--color-text)', fontSize: 12, fontFamily: 'monospace', fontWeight: 600, flex: 1 }}>
                    {entry?.label ?? 'CONFIG'}
                </span>
                <button
                    onClick={onClose}
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: 14, display: 'flex', flexDirection: 'column', gap: 14 }}>
                {entry?.configSchema.length === 0 && (
                    <p style={{ color: 'var(--color-text-muted)', fontSize: 12, fontFamily: 'monospace' }}>
                ...
                {entry?.configSchema.map(field => (
                    <div key={field.key}>
                        <label style={{
                            display: 'block', color: 'var(--color-text-muted)', fontSize: 11,
